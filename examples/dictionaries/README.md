...
各ディレクトリは `intents.tsv` / `slots.tsv` / `intent.properties` / `synonyms.tsv`（同義語レイヤ）の
セット（書式はルートの README を参照）。各意図は **約11例**（敬体/常体/口語/言い換え）で構成。

`synonyms.tsv` は推論前に「払い出し→出庫」「お休み→有給休暇」のように variant を canonical へ正規化する
**小さなドメイン同義語辞書**（数行〜十数行）。辞書に無い言い回しでも canonical 形に揃えてから照合できる。
...
## A/B 実験：同義語レイヤ vs 汎用埋め込み(chiVe)

語彙をわざとずらした**パラフレーズ11件**で「言い換え耐性」を比較した結果：

| | TF‑IDF 単体 | **TF‑IDF + synonyms.tsv** | chiVe埋め込み (549MB) |
|---|:--:|:--:|:--:|
| ドメイン内 40件 | 40/40 | **40/40** | 38/40 |
| パラフレーズ 11件 | 5/11 | **9/11** | 7/11 |
| 追加サイズ | – | **約12行/ドメイン (KB)** | 549MB + Sudachi辞書 |
| 決定論 / 閾値 / 説明性 | ◎ | ◎ | △（cosが高止まりし閾値が効きにくい） |

**狭いドメインでは、十数行の同義語辞書が巨大な汎用埋め込みを両軸で上回る**（決定論的・KBサイズのまま）。
ポイントは canonical を**辞書の実語彙に合わせる**こと（例: `機密保持`→`秘密保持`）。
残差（CHECK_BUDGET の曖昧文、REPORT_INCIDENT↔REPORT_OUTAGE の本質的重複）は例文追加や意図設計で対応する。
